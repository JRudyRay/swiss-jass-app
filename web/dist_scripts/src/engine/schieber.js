...
exports.deal = deal;
exports.startGameLocal = startGameLocal;
exports.startNewHand = startNewHand;
exports.chooseRandomTrump = chooseRandomTrump;
exports.chooseBotTrump = chooseBotTrump;
exports.setTrumpAndDetectWeis = setTrumpAndDetectWeis;
exports.getLegalCardsForPlayer = getLegalCardsForPlayer;
exports.peekTrickWinner = peekTrickWinner;
exports.playCardLocal = playCardLocal;
exports.detectWeis = detectWeis;
exports.calculateTeamWeis = calculateTeamWeis;
exports.resolveTrick = resolveTrick;
exports.chooseBotCard = chooseBotCard;
...
function rankValue(rank, isTrump) {
    if (isTrump)
        return trumpOverride[rank];
...
const trumpOrder = ['U', '9', 'A', '10', 'K', 'O', '8', '7', '6'];
const normalOrder = ['A', '10', 'K', 'O', 'U', '9', '8', '7', '6'];
...
function makeId(suit, rank) { return `${suit}_${rank}_${Math.random().toString(36).slice(2, 9)}`; }
function createDeck() {
    const deck = [];
    for (const s of suits)
        for (const r of ranks)
            deck.push({ id: makeId(s, r), suit: s, rank: r });
    return deck;
}
...
function startNewHand(previousState) {
    const players = deal();
    ...
    hand.forEach(card => {
        ...
        switch (card.rank) {
            ...
            case 'U':
                ...
                break; // Jack is strong in trump
            case '10':
                suitStrength[card.suit] += 2;
                break;
            case '9':
                suitStrength[card.suit] += 1;
                break; // 9 is strong in trump
        }
    });
...
function peekTrickWinner(state) {
    ...
    const winnerIdx = winnerOfTrick(state.currentTrick, state.trump || undefined, lead);
    const winnerCard = state.currentTrick[winnerIdx];
    return winnerCard.playerId;
}
function playCardLocal(state, playerId, cardId) {
    const st = JSON.parse(JSON.stringify(state)); // naive clone
...
function calculateTeamWeis(players) {
    ...
    if (team1BestWeis && team2BestWeis) {
        if (isWeisBetter(team1BestWeis, team2BestWeis)) {
            ...
            team1Points = team1Players.reduce((sum, p) => { var _a; return sum + (((_a = p.weis) === null || _a === void 0 ? void 0 : _a.reduce((s, w) => s + w.points, 0)) || 0); }, 0);
        }
        else if (isWeisBetter(team2BestWeis, team1BestWeis)) {
            // Team 2 wins, gets all their Weis points
            team2Points = team2Players.reduce((sum, p) => { var _a; return sum + (((_a = p.weis) === null || _a === void 0 ? void 0 : _a.reduce((s, w) => s + w.points, 0)) || 0); }, 0);
        }
...
function resolveTrick(state) {
    ...
    const winnerIdx = winnerOfTrick(st.currentTrick, st.trump || undefined, lead);
    ...
    // Apply contract multiplier (assumption: multiplier applies to trick points and final bonuses)
    const multiplier = st.trumpMultiplier || 1;
    trickPoints = trickPoints * multiplier;
    const winnerTeam = st.players.find(p => p.id === winnerPlayer).team;
    if (winnerTeam === 1)
        st.scores.team1 += trickPoints;
    else
        st.scores.team2 += trickPoints;
    // Add Weis points if this is the last trick
    if (isLastTrick) {
        const weisScore = calculateTeamWeis(st.players);
        // Assume Weis points are also affected by the contract multiplier
        st.scores.team1 += (weisScore.team1 || 0) * multiplier;
        st.scores.team2 += (weisScore.team2 || 0) * multiplier;
    }
    st.currentTrick = [];
    st.trickLead = null;
    ...
    if (st.players.every(p => p.hand.length === 0)) {
        ...
        const team1Score = st.scores.team1;
        const team2Score = st.scores.team2;
        // Check for match-all (one team captured all tricks) and award match bonus
        try {
            const team1Cards = st.players.filter(p => p.team === 1).reduce((s, p) => { var _a; return s + (((_a = p.tricks) === null || _a === void 0 ? void 0 : _a.length) || 0); }, 0);
            const team2Cards = st.players.filter(p => p.team === 2).reduce((s, p) => { var _a; return s + (((_a = p.tricks) === null || _a === void 0 ? void 0 : _a.length) || 0); }, 0);
            const matchBonus = st.matchBonus || 100;
            if (team1Cards === 36) {
                st.scores.team1 += (matchBonus * (st.trumpMultiplier || 1));
            }
            else if (team2Cards === 36) {
                st.scores.team2 += (matchBonus * (st.trumpMultiplier || 1));
            }
        }
        catch (e) {
            // ignore
...
function chooseBotCard(state, botId) {
    ...
    const isPartnerWinning = partner && (currentWinner === null || currentWinner === void 0 ? void 0 : currentWinner.playerId) === partner.id;
    if (isPartnerWinning) {
...
function canBotWinTrick(legal, trick, trumpContract, leadSuit) {
    if (trick.length === 0)
        return legal; // First card always "wins" initially
